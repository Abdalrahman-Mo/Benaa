  ...
  const featuredCourses = [
    {
      ...
      description: 'تعلم أساسيات التصميم المعماري وأهم البرامج المستخدمة في المجال',
    ...
    },
    {
      ...
      description: 'تعلم إدارة المشاريع بفعالية باستخدام أحدث الأساليب والتقنيات',
      thumbnail: '/placeholder.svg',
      lessons: 28,
      certificate: true,
      discount: 18,
    },
    {
      ...
      description: 'تخصص في التحليل الإنشائي وتصميم المنشآت الخرسانية والمعدنية',
      thumbnail: '/placeholder.svg',
      lessons: 42,
      certificate: true,
      discount: 17,
        ...
        <div className="container-custom">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-muted-foreground font-cairo leading-relaxed mb-8">
              طور مهاراتك في البناء والتشييد مع خبراء المجال واحصل على شهادات معتمدة
            </p>
            ...
            {stats.map((stat, index) => (
              <Card key={index} className="text-center">
                <CardContent className="p-6">
                  ...
                  <p className="font-semibold text-foreground mb-1 font-cairo">{stat.title}</p>
                  <p className="text-sm text-muted-foreground font-cairo">{stat.description}</p>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <div className="container-custom">
          <Tabs defaultValue="courses" className="w-full">
            <TabsList className="grid w-full grid-cols-4 max-w-3xl mx-auto mb-12">
              <TabsTrigger value="courses" className="font-cairo">الدورات المميزة</TabsTrigger>
              <TabsTrigger value="categories" className="font-cairo">التخصصات</TabsTrigger>
              <TabsTrigger value="paths" className="font-cairo">المسارات</TabsTrigger>
              <TabsTrigger value="instructors" className="font-cairo">المدربون</TabsTrigger>
            </TabsList>
                    ...
                    <Card key={index} className="hover:shadow-elegant transition-shadow overflow-hidden">
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between mb-2">
                          <Badge variant="outline" className="font-cairo">{course.level}</Badge>
                          <div className="flex items-center">
                            <Star className="w-4 h-4 text-yellow-500 fill-current" />
                            <span className="mr-1 text-sm">{course.rating}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-4 space-x-reverse mb-4">
                          ...
                          <div>
                            <p className="text-sm font-semibold font-cairo">{course.instructor}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            <span className="text-2xl font-bold text-primary font-cairo">{course.price}</span>
                  ...
                  {learningPaths.map((path, index) => (
                    <Card key={index} className="hover:shadow-elegant transition-shadow">
                      <CardContent className="p-8">
                        <div className="flex items-center justify-between mb-4">
                          <Badge className="bg-primary/10 text-primary font-cairo">مسار متكامل</Badge>
                          <div className="text-left">
                            <p className="text-2xl font-bold text-primary font-cairo">{path.price}</p>
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-6 text-center">
                          <div>
                            <p className="text-sm text-muted-foreground font-cairo">الدورات</p>
                            <p className="font-bold font-cairo">{path.courses}</p>
                          </div>
                          <div>
                            <p className="text-sm text-muted-foreground font-cairo">المدة</p>
                            <p className="font-bold font-cairo">{path.duration}</p>
                          </div>
                          <div>
                            <p className="text-sm text-muted-foreground font-cairo">المستوى</p>
                            <p className="font-bold font-cairo">{path.level}</p>
                          </div>
                        </div>
                        
                        <div className="space-y-3 mb-6">
                          <p className="font-semibold font-cairo">محتوى المسار:</p>
                          {path.steps.map((step, stepIndex) => (
                            <div key={stepIndex} className="flex items-center space-x-3 space-x-reverse">
                    ...
                    <Card key={index} className="hover:shadow-elegant transition-shadow">
                      <CardContent className="p-6 text-center">
                        ...
                        <div className="flex items-center justify-center mb-4">
                          <Star className="w-4 h-4 text-yellow-500 fill-current" />
                          <span className="mr-1 font-semibold">{instructor.rating}</span>
                          <span className="text-sm text-muted-foreground font-cairo">تقييم</span>
                        </div>
                        
                        <div className="grid grid-cols-3 gap-4 mb-6 text-center">
                          <div>
                            <p className="text-lg font-bold">{instructor.experience}</p>
                            <p className="text-xs text-muted-foreground font-cairo">خبرة</p>
                          </div>
                          <div>
                            <p className="text-lg font-bold">{instructor.courses}</p>
                            <p className="text-xs text-muted-foreground font-cairo">دورات</p>
                          </div>
                          <div>
                            <p className="text-lg font-bold">{instructor.students}</p>
                            <p className="text-xs text-muted-foreground font-cairo">طالب</p>
                          </div>
                        </div>
      ...
      <section className="py-16 bg-primary text-primary-foreground" dir="rtl">
        <div className="container-custom text-center">
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button size="lg" variant="outline" className="border-primary-foreground text-primary-foreground hover:bg-primary-foreground hover:text-primary font-cairo px-8">
              تجربة مجانية
            </Button>
          </div>
        </div>
